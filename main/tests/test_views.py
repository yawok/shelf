...
class TestProductListPage(TestCase):
    ...
    def test_product_page_filters_by_tag_and_active(self):
        ...
        product.tags.create(name="Open Source", slug="open-source")
        models.Product.objects.create(
            name="Cell",
            price=Decimal("10.00"),
        )
...
class TestUserCreationForm(TestCase):
    ...
    def test_signup_page_submition_works(self):
        ...
        with patch.object(forms.UserCreationForm, "send_mail") as mock_send:
            ...
            self.assertTrue(
                models.User.objects.filter(email="user@example.com").exists()
            )
...
class TestAddressViews(TestCase):
    def test_address_list_page_returns_only_owned(self):
        user1 = models.User.objects.create_user("astro", "random67")
        user2 = models.User.objects.create_user("kenneth", "kenneth's account432")
    ...
    def test_address_create_stores_user(self):
        user = models.User.objects.create_user("randy", "randomRandy443")
        post_data = {
...
class TestAddToBasketView(TestCase):
    def test_add_to_basket_logged_in_works(self):
        ...
        cb = models.Product.objects.create(
            name="The Cathedral and the bazaar", slug="cathedral-bazaar", price="23.00"
        )
        pcc = models.Product.objects.create(
            name="Python Crash Course", slug="python-crash-course", price="12.00"
        )
        self.client.force_login(user)
...
class TestAddToBasketSignal(TestCase):
    def test_add_to_basket_login_merge_works(self):
        ...
        response = self.client.post(
            reverse("login"), {"email": "random@person.com", "password": "randomPer5on"}
        )
